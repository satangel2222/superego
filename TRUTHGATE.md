...
| :--- | :--- | :--- |
| **电商与支付**<br>(Shopify / Stripe) | 修改本地结账逻辑、写 webhook 处理函数、测试汇率转换算法、跑本地单测 | 调整线上真实商品售价、在 Stripe 后台给真实客户退款、下架真实商品 |
| **社媒与通信**<br>(Telegram / WhatsApp) | 重构本地 Bot 消息队列、编写防防刷限流器、配置本地 .env、修复 parser 语法 | 向真实频道/用户群发广播、删除真实外部群聊消息、向客户发起付费会话 |
| **云端与数据**<br>(AWS / Fly.io / Postgres) | 编写 Dockerfile、修改 fly.toml、编写本地 migration 脚本、自测 SQL 查询 | 执行云端 `fly deploy` 生产上线、清空/DROP 生产数据库、修改云端 IAM 密钥 |
| **酒旅与商业**<br>(Airbnb / Booking / OTA) | 编写看板本地抓取脚本、重构房东盈亏算法、修复前端图表渲染、生成离线报表 | 修改线上真实房源日历（开房/关房）、修改挂牌价格、取消真实房客订单 |
| **多媒体与工具**<br>(BananaFlow / Zimagen / CLI) | 重构逆向爬虫、优化图像后处理算法、跑集成测试、修复 ffmpeg 命令行参数 | 调用需要付费扣款的第三方高价商业 API、删除外部持久化未备份的原图 |
...
---
*本文于 2026-09-23 全量落盘，同步覆盖四端环境，作为全局唯一单源物理真相长期生效。*
