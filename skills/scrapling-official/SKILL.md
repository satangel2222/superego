...
description: >-
  Scrape web pages using Scrapling with anti-bot bypass (like Cloudflare Turnstile), stealth headless browsing, spiders framework, adaptive scraping, and JavaScript rendering. Use when asked to scrape, crawl, or extract data from websites; web_fetch fails; the site has anti-bot protections; write Python code to scrape/crawl; or write spiders. ⛔⛔ 跑它必须用 `py -3`(= 本机 3.14,实测装着 **scrapling 0.4.12**)——**`py -3.12` 里没有,会报 ModuleNotFoundError**。2026-08-26 实录:我抓不到一个动态网页时先说「本机没有能渲染 JS 的工具」、又去研究要不要花钱装 firecrawl,**而它一直装着,只是我习惯写 `py -3.12` 所以看不见它**。⛔ 同一环境里 `cloakbrowser 0.5.8` 也在(要登录+会弹滑块的站优先用它,见 `~/.claude/browser-routing.md`)。
---
